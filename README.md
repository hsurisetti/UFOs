...
### Drawback :
  Some drawbacks which I noticed are 
   - If the user doesn't know the exact date but knows the month that needs to be searched upon, the user cannot search specifically for that month. The user needs to know the exact full date.
...
My recommendations would be to
 - Provide a data range which looks for all the data within that range if the user wants to look for any specific month of data
 ...
 - Provide a drop-down option for the list of shapes which gives the user with the option of different shapes that user can select from.
